...
namespace FerroJson.DynamicDictionary
{
	/// <summary>
	...
	public class DynamicDictionary : DynamicObject, IEquatable<DynamicDictionary>, IHideObjectMembers, IEnumerable<string>, IDictionary<string, object>
	{
		...
		/// <summary>
		/// Creates a dynamic dictionary from an <see>
		///         <cref>IList{KeyValuePair{TKey,TValue}}</cref>
		///     </see>
		///     instance.
		/// </summary>
		/// <param name="values">An <cref>IList{KeyValuePair{TKey,TValue}}</cref> instance, that the dynamic dictionary should be created from.</param>
		/// <returns>A <see cref="DynamicDictionary"/> instance.</returns>
		public static DynamicDictionary Create(IList<KeyValuePair<string, dynamic>> values)
		{
			var instance = new DynamicDictionary();

			foreach (var keyValuePair in values)
			{
				instance.Add(keyValuePair);
			}
